class WorkoutsetsController < ApplicationController
  ...
  def show
    ...
    @graph = Workoutset.joins(:workout).where(workouts: {profile: @set.workout.profile, is_template: false}, exercice: @set.exercice).where.not(total_weight: 0).order("workouts.date desc").limit(10).map { |set| [set.workout.date.strftime("%d/%m"), set.total_weight]}.reverse
  end
